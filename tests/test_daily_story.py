...
class DailyStoryTest(unittest.TestCase):
 ...
 def test_opening_primary_elements_have_no_fade(self):
  ...
  self.assertIn('opening_fade = "" if start == 0',source)
 def test_opening_region_contract_requires_visible_headline_panel_and_hero(self):
  source=(ROOT/"scripts/check_opening_frames.py").read_text()
  for check in ("headline_area_not_empty", "headline_panel_present", "headline_area_contrast", "accent_line_present", "hero_visual_present"):
   self.assertIn(f'checks[f"frame_{{label}}s_{check}"]',source)
