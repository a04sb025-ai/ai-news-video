...
class AdaptiveExplainerTest(unittest.TestCase):
    ...
    def test_adaptive_renderer_uses_single_pass_final_wav_alignment(self):
        renderer = (ROOT / "scripts/render_adaptive_explainer.py").read_text()
        ...
        self.assertIn('narration.write_text(" ".join(narration_chunks) + "\\n")', renderer)
        ...
        self.assertIn('"audio_pipeline": "single-pass-open-jtalk-audio-led-v3"', renderer)
        self.assertIn('"audio_single_pass": True', renderer)
        self.assertIn('"audio_alignment_source": "single-pass-wav-silence-snapped"', renderer)
        self.assertIn('"audio_detected_silence_regions": [', renderer)
        self.assertIn('"audio_boundary_alignment": boundary_alignment', renderer)
        self.assertIn('"audio_tempo_adjustment": False', renderer)
        self.assertNotIn('"audio_alignment_source": "probe-duration-proportional"', renderer)
    ...
    def test_rendered_timing_metadata_and_images_follow_measured_audio(self):
        ...
        self.assertIn('timeline = [0.0, *scene_boundaries, narration_duration + FINAL_END_PAUSE_SECONDS]', adaptive)
