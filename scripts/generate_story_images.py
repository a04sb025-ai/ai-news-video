#!/usr/bin/env python3
"""Generate at most four cached editorial images for a story."""
...
def generate(destination, prompt, api_key):
    if destination.is_file() and destination.stat().st_size > 0:
        ...
        return "reused"
    payload = json.dumps({"model": CONFIG["model"], "prompt": prompt, "size": CONFIG["size"],
                          "quality": CONFIG["quality"], "n": 1, "output_format": "png"}).encode()
    ...
    if not data: raise RuntimeError("image provider returned no PNG data")
    temporary = destination.with_suffix(".png.part")
    temporary.write_bytes(base64.b64decode(data, validate=True)); temporary.replace(destination)
